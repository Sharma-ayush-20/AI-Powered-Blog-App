...
function BlogTableItem({ blog, fetchBlogs, index }) {
    ...
    const togglePublish = async () => {
        try {
            const response = await axios.post(`/api/blog/toggle-publish`, { id: blog._id })
            if (response.data.success) {
                toast.success(response.data.message)
                await fetchBlogs()
            }
    ...
    return (
        <tr className='border-y border-gray-300'>
            ...
            <td className='px-2 py-4 max-sm:hidden'>
                <p className={`${blog.isPublished ? "text-emerald-600" : "text-orange-700"}`}>{blog.isPublished ? 'Published' : 'Unpublished'}</p>
            </td>
